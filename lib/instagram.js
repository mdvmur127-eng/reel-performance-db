...
function isReelMedia(item) {
  ...
  return mediaType === "REELS" || (mediaType === "VIDEO" && mediaProductType === "REELS");
}

async function fetchAllReels({ igUserId, accessToken, maxItems = 20 }) {
  ...
  let afterCursor = "";
  const reels = [];
  const reelLimit = Math.max(1, Math.round(Number(maxItems) || 20));
  while (true) {
    ...
    for (const item of items) {
      if (!isReelMedia(item)) continue;
      reels.push(item);
      if (reels.length >= reelLimit) break;
    }
  ...
  return reels.slice(0, reelLimit);
}
