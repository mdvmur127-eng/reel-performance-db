...
begin
  ...
  if exists (
    select 1
    from information_schema.columns
    where table_schema = 'public' and table_name = 'reels' and column_name = 'ig_media_id'
  ) then
    update public.reels
    set ig_media_id = coalesce(ig_media_id, instagram_media_id)
    where ig_media_id is null;
  end if;

  if exists (
    select 1
    from information_schema.columns
    where table_schema = 'public' and table_name = 'reels' and column_name = 'saved'
  ) then
    update public.reels
    set saved = coalesce(saved, saves, 0);
  end if;

  if exists (
    select 1
    from information_schema.columns
    where table_schema = 'public' and table_name = 'reels' and column_name = 'plays'
  ) then
    update public.reels
    set plays = coalesce(plays, views, 0);
  end if;
  ...
  if exists (
    ...
    update public.reels
    set posted_at = coalesce(posted_at, published_at, created_at)
...
with dedupe as (
  select
    id,
    row_number() over (
      partition by user_id, ig_media_id
...
create unique index if not exists reels_user_instagram_media_uidx
on public.reels (user_id, instagram_media_id)
where instagram_media_id is not null;

create unique index if not exists reels_user_ig_media_uidx
